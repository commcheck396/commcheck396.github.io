...
![pic from internet](http://commcheck396.github.io/assets/img/2022_2_14/contrast.png)
...
![pic from internet](http://commcheck396.github.io/assets/img/2022_2_14/one-hot.png)
```python
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(  # input shape (1, 28, 28)
            nn.Conv2d(
                ...
                out_channels=16,    # n_filters,filter个数
                kernel_size=5,      # filter size
                stride=1,           # filter movement/step，步长
                padding=2,      # 如果想要 con2d 出来的图片长宽没有变化, padding=(kernel_size-1)/2 当 stride=1  不是很懂
            ),      # output shape (16, 28, 28)
            nn.ReLU(),    # activation
            nn.MaxPool2d(kernel_size=2),    # 在 2x2 空间里向下采样, output shape (16, 14, 14)，由于经历了一个kerne_size为2的pooling，长宽变为之前的一半，经过16个filter，厚度增加16
        )
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = x.view(x.size(0), -1)   # 展平多维的卷积图成 (batch_size, 32 * 7 * 7)  啥玩意啊
        output = self.out(x)
        return output
